...
int total = 0;
vector<int> decrease;
for(unsigned int i = 0; i < contests.size(); i++){
    total += contests[i][0];
    if(contests[i][1])
        decrease.push_back(contests[i][0]);
}
//now we sort the important contests only to know which ones to lose
sort(decrease.begin(), decrease.end());
if(k > decrease.size()) return total;
int n = decrease.size() - k;
for(int i = 0; i < n; i++)
    total -= 2 * decrease[i];
return total;
}
